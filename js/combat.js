...
function combat_logic_intro() {
	...
	//boss battle music
	if(combat.enemy.type == ENEMY_BOSS)
		mazemap_set_music("boss");
	else
...
function combat_logic_input() {
  ...
  //TODO: set controls here, probably
  if (action_checkuse(BUTTON_POS_ATTACK)) {
    power_hero_attack();
	used_action = true;
  }
  else if (action_checkuse(BUTTON_POS_HEAL) && avatar.items.potions > 0 && avatar.spellbook >= 1 && avatar.hp < avatar.max_hp) {
    power_heal();
	used_action = true;
  }
  else if (action_checkuse(BUTTON_POS_BURN)) { //rock
	used_action = power_rock();
  }
  else if (action_checkuse(BUTTON_POS_UNLOCK)) { //battery
    used_action = power_electric();
  }
  else if (action_checkuse(BUTTON_POS_LIGHT)) { //water
    used_action = power_water();
  }
...
// determines whether the player receives a powerUp for the victory

function combat_determine_powerUp() {
    // determine if the player receives a powerUp
    
    //console.log("EXEC DETERMINE POWRUP");

    var powerUp_checker = Math.floor((Math.random() * 3) + 0); 
    
    //console.log(powerUp_checker);
    
    combat.powerUp_power = powerUp_checker;

    if (powerUp_checker == 3) {
        avatar.items.rocks++;
    } else if (powerUp_checker == 2) {
            avatar.items.cups++;
        } else if (powerUp_checker == 1) {
                avatar.items.batteries++;
            } else if (powerUp_checker == 0) {
                    avatar.items.potions++;
    }
}
...
function combat_render_victory() {
    ...
    if(combat.enemy.type == ENEMY_BOSS)
    {
        bitfont_render("You have destroyed", 80, 60, JUSTIFY_CENTER);
        bitfont_render("the evil!", 80, 70, JUSTIFY_CENTER);
        return;
    }
  ...
  treasure_render_item();
  powerUp_render_power(combat.powerUp_power);
  info_render_gold();
}
