...
var BUTTON_POS_BURN = {x:140, y:40, w:20, h:20}; //rock
var BUTTON_POS_UNLOCK = {x:120, y:60, w:20, h:20}; //battery
var BUTTON_POS_LIGHT = {x:140, y:60, w:20, h:20}; //water
...
function action_logic_moveselect() {
  ...
  // currently on the heal button
    ...
    else if (pressing.right && !input_lock.right && gamestate == STATE_COMBAT) {
      if (avatar.spellbook >= 2) {
        action.select_pos = BUTTON_POS_BURN;
        input_lock.right = true;
        redraw = true;
    ...
    else if (pressing.down && !input_lock.down && gamestate == STATE_COMBAT) {
