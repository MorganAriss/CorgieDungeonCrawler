 ...
 * Resolve power use
 */
//TODO: hero and enemy elemental powers
...
function power_heal() {
  ...
  if (avatar.items.potions == 0) return;
...
//these run the check on ability to actually do the move in the function
//I should probably switch everything to the new call semantics, shouldn't I?
...
function power_rock() {
  ...
  if(avatar.items.rocks == 0) return false;
  
  combat.offense_action = "Throw rock!";
  ...
  // against electric, do double damage
  if (combat.enemy.category == ENEMY_CATEGORY_ELECTRIC)
  {
    attack_damage *= 2;
  }
  //against water, do half damage
  else if (combat.enemy.category == ENEMY_CATEGORY_WATER)
  {
    attack_damage = Math.round(attack_damage / 2);
  }

  avatar.items.rocks--;
  sounds_playSoundEx("throwrock");
...
//battery throw attack
function power_electric() {

  if(avatar.items.batteries == 0) return false;
  
  combat.offense_action = "Throw battery!";
  ...
  // against water, do double damage
  if (combat.enemy.category == ENEMY_CATEGORY_WATER)
  {
    attack_damage *= 2;
  }
  //against rock, do half damage
  else if (combat.enemy.category == ENEMY_CATEGORY_ROCK)
  {
    attack_damage = Math.round(attack_damage / 2);
  }

  avatar.items.batteries--;
  sounds_playSoundEx("throwbattery");
...
//cup of water throw attack
function power_water() {
